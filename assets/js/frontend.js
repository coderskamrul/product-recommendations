(($) => {
  ...
  var WCProductRecommendations = {
    ...
    bindEvents: function () {
      ...
      $(document.body).on(
        "updated_cart_totals",
        this.refreshCartRecommendations
      );
      $(document.body).on(
        "updated_checkout",
        this.refreshCheckoutRecommendations
      );
    ...
    refreshCartRecommendations: () => {
      var $container = $("#upspr-product-recommendations-cart");
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          action: "upspr_refresh_cart_recommendations",
    ...
    refreshCheckoutRecommendations: () => {
      var $container = $("#upspr-product-recommendations-checkout");
      if ($container.length === 0) {
        return;
      }
